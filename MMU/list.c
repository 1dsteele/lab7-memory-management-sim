...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
  ...
  */
  ...
  // if head doesn't exist or newblk is smaller than head
  if (!current || newblk->start < current->blk->start) {
  ...
  while (current != NULL) {
    if ((newblk->start > current->blk->start) && (!current->next || newblk->start <= current->next->blk->start)) {
      node_t *temp = current->next;
      current->next = newblk_node;
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
  ...
  if (!current || compareSize(blocksize, current->blk)) {
    list_add_to_front(l, newblk);
    return;
  }
  ...
  while (current != NULL) {
    if ((!compareSize(blocksize, current->blk)) && (!current->next || compareSize(blocksize, current->next->blk))) {
      node_t *temp = current->next;
      current->next = newblk_node;
      newblk_node->next = temp;
      return;
...
void list_coalese_nodes(list_t *l){ 
  ...
  while (list_length(l) != 0 && prev && current) {
